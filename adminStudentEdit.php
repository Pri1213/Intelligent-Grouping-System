...
$sql_user = "SELECT UserType FROM user WHERE UserID = :userID";
$statement_user = $conn->prepare($sql_user);
$statement_user->bindParam(":userID", $userID);
$statement_user->execute();
$userType = $statement_user->fetchColumn();

// If the user is not a student, redirect them to an error page
if ($userType !== "student") {
  header('Location: adminDashStudent.php');
  exit();
}
// select all units from database
...
$sql = "SELECT u.UnitName FROM unit u JOIN stu_unit su ON u.UnitID = su.UnitID WHERE su.UserID = :userID;";
...
if (isset($_GET['add_unit'])) {
  ...
  $sql_check = "SELECT COUNT(*) FROM stu_unit WHERE UserID = :userID AND UnitID = :unitID";
  ...
  if ($count == 0) {
    $sql = "INSERT INTO stu_unit (UserID, UnitID) VALUES (:userID, :unitID)";
    $statement = $conn->prepare($sql);
    $statement->bindParam(":userID", $userID);
  ...
  header('Location:adminStudentEdit.php?id=' . $userID);
  ;
}
if (isset($_GET['remove_unit'])) {
  ...
  $sql = "DELETE FROM stu_unit WHERE UserID = :userID AND UnitID = :unitID";
  $statement = $conn->prepare($sql);
  $statement->bindParam(":userID", $userID);
  $statement->bindParam(":unitID", $unitID);
  $statement->execute();
...
<body>
  ...
  <header class="siteheadertop">
    ...
    <nav class="headernavigation">
      <ul>
        ...
        <li><a href="adminDashStudent.php">Back</a></li>
  ...
  <div class="container">
    <br>
    <h1>Student Unit Selection Form</h1>
    <form method="GET" action="adminStudentEdit.php">
